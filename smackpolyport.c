...
struct peer {
	...
	char		smack[SMK_LABELLEN];	/* Smack label */
	int		port;			/* TCP port */
...
struct peer *alloc_peer(char *smack, int port, int socket, int list)
{
	...
	if (smack == NULL)
		...
		strncpy(result->smack, smack, SMK_LABELLEN);
	result->port = port;
...
main(int argc, char *argv[])
{
	...
	for (;;) {
		 ...
		 */
		len = SMK_LABELLEN;
		if (getsockopt(star_socket, SOL_SOCKET, SO_PEERSEC,
				client->smack, &len) < 0) {
			perror("getsockopt SP_PEERSEC");
			exit(1);
		}
